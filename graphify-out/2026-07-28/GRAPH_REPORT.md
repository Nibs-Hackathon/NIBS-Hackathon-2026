...
## Corpus Check
- 333 files · ~181,279 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2494 nodes · 4391 edges · 201 communities (169 shown, 32 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 224 edges (avg confidence: 0.52)
...
6. `get_session()` - 33 edges
7. `ComputationEngine` - 33 edges
...
## Communities (201 total, 32 thin omitted)

### Community 0 - "catalog/index.js"
Cohesion: 0.07
Nodes (69): DecisionButtonGroup(), FilterChipBar(), PrimaryCTA(), RationaleField(), ScenarioSlider(), EmptyState(), ForecastChart(), HealthRing() (+61 more)

### Community 1 - "KeyStatus"
Cohesion: 0.08
Nodes (12): KeyStatus, any, RateLimiter, Track status of each API key., Quarantine a quota-limited key until Gemini says it can be retried., ✅ Load API keys from ALL possible sources., ✅ Get the next available API key with rotation., Extract Gemini's retry delay without exposing provider error text. (+4 more)
...
Cohesion: 0.05
Nodes (44): get_agent_metrics(), get_agents(), Agent adapter using BackendAPI., Return monitor metrics calculated from the live state., Return registered agents and their latest execution state., get_assets(), Asset adapter using BackendAPI., Get all assets with telemetry history. (+36 more)
...
### Community 5 - "main.py"
Cohesion: 0.09
Nodes (39): list_asset_notes(), get_asset_health(), Return identity, health, and recent readings for one asset., Return normalized Neon retrieval results from the registered KnowledgeAgent., search_knowledge(), database_health(), export_report(), get_agent_activity() (+31 more)

### Community 6 - "Task"
Cohesion: 0.08
Nodes (30): ask_knowledge_agent(), _emit(), generate_conversational_response(), get_knowledge_agent(), is_operational_query(), KnowledgeAgentUnavailable, ProgressCallback, RuntimeError (+22 more)
...
Cohesion: 0.12
Nodes (18): Agent, ABC, Every agent must return an AgentResult., Production Diagnostic Agent with dynamic thresholds., Knowledge agent shared by MAO workflows and Command Nexus., Production Maintenance Agent with dynamic priority levels., Runtime notification agent with dynamic severity levels., Production Planning Agent with Gemini-generated sequences. (+10 more)

### Community 10 - "ActionDB"
Cohesion: 0.07
Nodes (30): KnowledgeSearchError, RuntimeError, Read-only Knowledge Base access through the shared MAO kernel., Raised when the registered knowledge retrieval path is unavailable., Return normalized Neon retrieval results from the registered KnowledgeAgent., search_knowledge(), approve_maintenance_work_order(), AssetNoteRequest (+22 more)

### Community 11 - "views/shared.jsx"
Cohesion: 0.18
Nodes (11): normalizeAgentActivityRow(), ExportAuditButton(), MetricWithProvenance(), MissionControlOS(), averageHealth(), EvidenceItem(), Health(), MiniGraph() (+3 more)
...
Cohesion: 0.06
Nodes (26): Health prediction using computation engine., calculate_severity(), get_incidents(), Incident adapter with dynamic severity calculation from config., Get all incidents from the runtime., Calculate severity from event payload using dynamic thresholds from AI config., Health prediction using computation engine., AIConfigGenerator (+18 more)

### Community 14 - "client.js"
Cohesion: 0.13
Nodes (18): api, approveWorkOrder(), askAssistant(), createWorkOrder(), getMaintenancePlan(), getPredictions(), Assistant(), prompts (+10 more)

### Community 15 - "ProductShell.jsx"
Cohesion: 0.10
Nodes (29): getOperationsLive(), markNotificationsRead(), facilityOptionsFromRefineries(), App(), createProductTheme(), PRODUCT_PATHS, RoutedApp(), ApplicationShell() (+21 more)

### Community 16 - "ConfigService"
Cohesion: 0.10
Nodes (17): _ApiProxy, _get_api_instance(), Unified backend API for frontend access with caching and refinery support., ConfigService, Get thresholds - uses precomputed values for speed., Generate agent sequence for an incident type., Generate and cache operational configurations using Gemini., Generate priority level for an incident. (+9 more)

### Community 17 - "AssetConsole.jsx"
Cohesion: 0.17
Nodes (22): getAssetHealth(), getAssetNotes(), getTwinAssets(), saveAssetNote(), mergeAssetsWithTwin(), normalizeTwinAsset(), readWorkspace(), useWorkspace() (+14 more)
...
Cohesion: 0.18
Nodes (7): Asset note persistence helpers., Replace the latest note for an asset (single active note per asset for UI)., upsert_asset_note(), save_asset_note(), Runtime-only notification model for MAO workflow outputs., test_runtime_incident_includes_workflow_confidence(), datetime

### Community 20 - "operations_adapter.py"
Cohesion: 0.21
Nodes (23): _action_step(), _agent_step(), get_execution_report_export(), get_execution_reports(), get_incident_audit(), get_incident_audit_detail(), get_live_investigation(), get_operations_live() (+15 more)
...
### Community 23 - "models/sensor.py"
Cohesion: 0.20
Nodes (9): BaseModel, Enum, str, Sensor, SensorType, Manual incident injection for the shared live simulator., Asset, Generate telemetry for one tick. (+1 more)
...
Cohesion: 0.13
Nodes (11): ExecutionContext, Register an agent result and update execution state., ExecutionReport, BaseModel, Enum, str, TaskStatus, Orchestrator (+3 more)
...
Cohesion: 0.17
Nodes (25): getAgentMetrics(), getAgents(), getIncidentAuditDetail(), getTelemetry(), searchKnowledge(), triggerIncident(), decisionEntriesFromIncident(), incidentTelemetryWindow() (+17 more)

### Community 31 - "adapters/agent_activity_adapter.py"
Cohesion: 0.29
...
### Community 33 - "persistence.py"
Cohesion: 0.09
Nodes (13): is_database_configured(), True when DATABASE_URL was provided and a session factory exists., AgentExecutionDB, AssetNoteDB, ExecutionReportDB, IncidentDB, Operator shift notes attached to an asset (Phase 3 persistence)., AgentRepository (+5 more)

### Community 34 - "BackendAPI"
Cohesion: 0.16
Nodes (6): BackendAPI, Get health for a specific asset., Single interface for frontend to access backend data with caching., Get Gemini-generated workflow sequence., Get current simulation status., Lazy load runtime to avoid circular imports.
...
### Community 38 - "_LocalVectorStore"
Cohesion: 0.11
Nodes (15): MaintenanceAgent, Maintenance agent using Gemini-generated priorities., PredictionAgent, Extract asset type from context., Estimate health and risk from telemetry using dynamic thresholds., Calculate degradation rate using dynamic thresholds., Compile prior AgentResult objects with dynamic formatting., ReportAgent (+7 more)

### Community 39 - "PersistenceService"
Cohesion: 0.09
Nodes (15): TelemetryDB, Create many telemetry readings in smaller batches., TelemetryRepository, get_persistence(), PersistenceService, Save execution to database synchronously., Persist physical incident resolution independently of MAO completion., Handles database persistence with buffer and sync saving. (+7 more)

### Community 40 - ".__init__"
Cohesion: 0.14
Nodes (7): EventBus, Planner, Any, mao/workflows/planner.py  Workflow Planner  Determines which workflow should, Selects the most appropriate workflow for an incoming event., Determine which workflow should handle an event.          Priority:, Infer workflow directly from telemetry.

### Community 41 - "kernel.py"
Cohesion: 0.18
...
Cohesion: 0.10
Nodes (11): get_ai_config(), get_runtime(), Services module exports - NO circular imports., Get the AI Configuration Generator (singleton)., get_kernel(), get_simulator(), Return the timer-driven simulator without creating a second one., Lazy-initialize and return the shared MAO kernel. (+3 more)

### Community 47 - "IncidentService"
Cohesion: 0.15
Nodes (12): Trigger an incident and return formatted results., trigger_incident(), calculate_severity(), get_incidents(), Incident adapter with dynamic severity calculation from config., Trigger an incident and return formatted results., Get all incidents from the runtime., Calculate severity from event payload using dynamic thresholds from AI config. (+4 more)
...
### Community 49 - ".compute_asset"
Cohesion: 0.14
Nodes (9): Any, Calculate health based on telemetry violations., Calculate degradation rate from telemetry trends., Calculate Remaining Useful Life in days., Calculate confidence based on sample count., Determine status from health value., Return cached results if computation not needed., Compute everything for all assets. (+1 more)

### Community 50 - "ComputationEngine"
Cohesion: 0.11
Nodes (10): ComputationEngine, Computes health, failure probability, RUL in real-time., Calculate failure probability from health and degradation., Get cached health for an asset., Get cached failure probability for an asset., Get cached RUL for an asset., Lazy load kernel to avoid circular imports., Check if computation should run (every 10-15 seconds). (+2 more)
...
Cohesion: 0.20
Nodes (7): Notification, NotificationService, NotificationSeverity, NotificationType, Enum, str, Real-time notification service.
...
Cohesion: 0.12
Nodes (11): MAOKernel, create_kernel(), get_kernel(), services/kernel_factory.py  Compatibility access point for the shared MAO kern, Return the already initialized production kernel., Return the shared MAO kernel instance., is_initialized(), Runtime module - lazy initialization with auto-start simulation. (+3 more)

### Community 62 - "HealthService"
Cohesion: 0.15
Nodes (7): HealthService, Health service using the computation engine with AI-generated thresholds., Calculate health from readings using AI-generated thresholds., Calculate health with custom limits (fallback method).                  Args:, Calculates asset health from recent telemetry using AI-generated thresholds., Get full health metrics for an asset using computation engine., Get cached health status for an asset.                  Args:             ass

### Community 63 - "Project_Code_Inventory_Updated.md"
Cohesion: 0.12
Nodes (9): backend/agents, backend/data/ai_config.json, Folder: backend/data Code Inventory, backend/data, backend/models, backend/rag, backend, Subfolders (+1 more)

### Community 64 - ".get_asset_telemetry"
Cohesion: 0.40
Nodes (3): _parse_iso(), Cached version of get_asset_telemetry., Get telemetry history for an asset; optional ISO since/until window.
...
### Community 67 - "Folder: backend/mao/core Code Inventory"
Cohesion: 0.18
...
### Community 68 - "Facility"
Cohesion: 0.27
Nodes (8): Facility, BaseModel, _auto_simulation_loop(), _initialize_simulator(), Background thread that keeps telemetry live and injects a demo incident every 1-, Initialize the simulator with all assets., Generate telemetry with optional targeted fault., SimulatedFacility

### Community 70 - "scripts"
Cohesion: 0.20
Nodes (9): name, private, scripts, backend, build, dev, lint, preview (+1 more)

### Community 71 - "KnowledgeSearchError"
Cohesion: 0.40
Nodes (4): KnowledgeSearchError, RuntimeError, Read-only Knowledge Base access through the shared MAO kernel., Raised when the registered knowledge retrieval path is unavailable.
...
Cohesion: 0.08
Nodes (16): backend/mao/__init__.py, backend/mao/kernel.py, backend/mao/orchestrator.py, Folder: backend/mao Code Inventory, backend/mao/models/execution_report.py, backend/mao/models/notification.py, backend/mao/models/result.py, backend/mao/models/task.py (+8 more)
...
### Community 86 - "get_health_prediction"
Cohesion: 0.50
Nodes (4): format_telemetry(), get_health_prediction(), Format telemetry for display., Get health prediction using the computation engine.
...
Cohesion: 0.09
Nodes (15): KnowledgeAgent, Retrieve refinery guidance for workflows and create grounded chat answers., Lazily load the persisted retrieval index and, for chat, Gemini., Support MAO workflow context and direct Command Nexus requests., Quota-aware Gemini embeddings with the same key rotation policy as chat., Compatibility export for the centralized LLM service., Compatibility export for the centralized LLM service., _has_invalid_gemini_proxy() (+7 more)
...
### Community 124 - "frontend_services/backend_api_new.py"
Cohesion: 0.50
...
### Community 160 - "src/api/FILES.md"
Cohesion: 0.40
Nodes (3): Folder: frontend/src/api Code Inventory, frontend/src/api/client.js, frontend/src/api

### Community 161 - "get_health_prediction"
Cohesion: 0.50
Nodes (4): format_telemetry(), get_health_prediction(), Get health prediction using the computation engine.      ``stress`` is a 0..1, Format telemetry for display.

### Community 162 - "Folder: backend/database Code Inventory"
Cohesion: 0.25
Nodes (8): backend/database/base.py, backend/database/bootstrap.py, backend/database/connection.py, backend/database/__init__database.py, backend/database/__init__.py, backend/database/models.py, backend/database/seed_demo.py, Folder: backend/database Code Inventory
...
### Community 165 - "database/README.md"
Cohesion: 0.10
...
Cohesion: 0.33
Nodes (9): exportReport(), getReports(), recordOperatorAction(), downloadReportExport(), downloadTextFile(), OperatorDecisionBar(), ExecutiveBriefing(), timelineFromReport() (+1 more)
...
## Knowledge Gaps
- **592 isolated node(s):** `workspaces`, `name`, `private`, `version`, `type` (+587 more)
...
- **Why does `ConfigService` connect `ConfigService` to `BackendAPI`, `BackendAPI`, `_LocalVectorStore`, `AgentResult`, `PlanningAgent`, `AIConfigGenerator`, `services/__init__.py`, `LLMManager`, `SafetyAgent`, `SimulatorController`, `DiagnosticAgent`, `NotificationAgent`, `frontend_services/backend_api_new.py`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `LLMManager` connect `LLMManager` to `KeyStatus`, `adapters/__init__.py`, `Task`, `AgentResult`, `AIConfigGenerator`, `services/__init__.py`, `ConfigService`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `AIConfigGenerator` connect `AIConfigGenerator` to `_LocalVectorStore`, `Simulator`, `services/__init__.py`, `IncidentService`, `LLMManager`, `ComputationEngine`, `RevenueService`, `runtime.py`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
