...
Contains 24 project file(s) directly in this folder (nested folders have their own inventory files).
...
class KeyStatus:
    ...
    def record_rate_limit(self, error: str, cooldown_seconds: float) -> None:
        """Quarantine a quota-limited key until Gemini says it can be retried."""
        self.failures += 1
        self.total_requests += 1
        self.last_error = error
        self.is_active = False
...
class LLMManager:
    ...
    def _load_keys(self) -> List[str]:
        ...
        # 1. Check comma-separated key pools before numbered variables.
        for var in MULTI_KEY_ENV_VARS:
            for key in (os.getenv(var) or "").split(","):
                key = key.strip()
                if key and key not in seen:
                    seen.add(key)
                    keys.append(key)
                    print(f"  Loaded key from {var}: {key[:8]}...{key[-4:]}")

        # 2. Check individually named environment variables.
                    ...
                    for line in f:
                        ...
                        if line.startswith('GEMINI_API_KEY') or line.startswith('GOOGLE_API_KEY'):
                            if '=' in line:
                                ...
                                candidates = value.split(",") if variable.strip() in MULTI_KEY_ENV_VARS else [value]
    ...
    def _retry_delay_seconds(error: str) -> float:
        """Extract Gemini's retry delay without exposing provider error text."""
            ...
            try:
                ...
                if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str:
                    ...
                    if key and key in self.key_statuses:
                        status = self.key_statuses[key]
                        status.record_rate_limit(error_str, self._retry_delay_seconds(error_str))
                else:
                    if key and key in self.key_statuses:
                        status = self.key_statuses[key]
